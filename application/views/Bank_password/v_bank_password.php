...
<script>
    $(document).ready(function() {
        ...
        // Start Input Select PIC User for Add Modal
        let selectedPics = [];
        ...
        $('#selectPic').on('change', function() {
            ...
            if (userId && !selectedPics.includes(userId)) {
                selectedPics.push(userId);
                updateBubbles();
            }

            select.value = ""; // reset dropdown setelah pilih
        });
        ...
        function updateBubbles() {
            ...
            selectedPics.forEach(id => {
                const userName = $(`#selectPic option[value="${id}"]`).text();

                const bubble = $(`<span class="badge bg-primary me-2 mb-2">${userName} 
                    <button type="button" class="btn-close btn-close-white btn-sm ms-1" data-id="${id}" aria-label="Remove"></button>
                </span>`);
        ...
        $('#picContainer').on('click', '.btn-close', function(e) {
            const id = $(this).data('id');
            selectedPics = selectedPics.filter(item => item !== id);
            updateBubbles();
        });
        // End Input Select PIC User for Add Modal

        // PIC Handling for Edit Modal
        ...
        $('#editSelectPic').on('change', function() {
            ...
            if (userId && !editSelectedPics.includes(userId)) {
        ...
        function updateEditBubbles() {
            ...
            editSelectedPics.forEach(id => {
                const userName = $(`#editSelectPic option[value="${id}"]`).text();
        ...
        $('#editPicContainer').on('click', '.btn-close', function(e) {
            const id = $(this).data('id');
            editSelectedPics = editSelectedPics.filter(item => item !== id);
            updateEditBubbles();
        });
        // End PIC Handling for Edit Modal
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.status === "success") {
                        ...
                        // Handle PICs
                        let picIds = data.pic_ids ? data.pic_ids.split(',') : [];
